...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/dashboard/connect?success=meta", request.url));
  } catch (err) {
    console.error("Meta OAuth callback error:", err);
    return NextResponse.redirect(new URL("/dashboard/connect?error=meta_exchange", request.url));
  }
}
